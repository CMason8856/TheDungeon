...
public class Panel extends JPanel {
    ...
    public void ruleTheFiefdom() {
        for (int x = 0; x <= MAX_WIDTH; x += mi) {
            for (int y = 0; y <= MAX_HEIGHT; y += mi) { // 0 = nothing, 1 = avatar, 2 = box, 3+ = whatever shape we implement next
    ...
    private static int collisionDetection(String dir) { //Uses dir to find out which direction it applies movement increment for the return
        ...
        switch(dir) { //first for loop gets the values for the side transversing, second one moves it a unit at a time until collision with a square in the vicinity (whole map for now)
            ...
            case "s": //S, DOWNWARDS, TO HELL WE GO!
                for (int i = 0; i < size - 2; i++) {
                    frontLine[i][0] = x + i;
                    frontLine[i][1] = y + 10; //who thought it made sense in GUI's to make positive incrementation of Y go down?
                }
                for (int a = 0; a < mi; a++) {
                    for (Box enemyOfTheFiefdom : Box.Box) { //checks to see if the new frontLine has crusaded into an enemy
                        for (int c = 0; c < enemyOfTheFiefdom.getWidth(); c++) {
